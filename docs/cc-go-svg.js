...
var MainGoSVG = function() {
	...
	window.document.addEventListener("DOMContentLoaded",function(event) {
		$global.console.info("MainGoSVG - " + model_constants_App.NAME + " Dom ready :: build: " + "2024-03-12 14:47:14" + " ");
...
MainGoSVG.prototype = {
	...
	,convertSrcToSVG: function() {
		...
		http.onError = function(error) {
			console.log("src/MainGoSVG.hx:83:","Failed to load SVG: " + error);
		};
		http.request();
	}
	,getDataSBG: function() {
		...
		var _range = { key : "transform-x", from : _this._isFrom ? 100 : objValue, to : !_this._isFrom ? 100 : objValue};
		...
		_this._transform.translate.x = 100;
		var _this1 = _this;
		var objValue = 0.0;
		if(_this1._target.hasAttribute("transform-y")) {
			objValue = parseFloat(_this1._target.getAttribute("transform-y"));
		}
		var _range = { key : "transform-y", from : _this1._isFrom ? -100 : objValue, to : !_this1._isFrom ? -100 : objValue};
		_this1._props.h["transform-y"] = _range;
		if(_this1._isFrom) {
			_this1.updateProperties(0);
		}
		if(_this1._transform.translate == null) {
			_this1._transform.translate = { x : 0};
		}
		_this1._transform.translate.y = -100;
		var _this = _this1;
		_this._isYoyo = true;
		var _this1 = _this;
		_this1._easing = cc_lets_easing_Quad.get_easeInOut();
		var _this = _this1;
		var x = null;
		var y = null;
		var key = _this.ROTATE;
		var objValue = 0.0;
		if(_this._target.hasAttribute(key)) {
			objValue = parseFloat(_this._target.getAttribute(key));
		}
		var _range = { key : key, from : _this._isFrom ? 10 : objValue, to : !_this._isFrom ? 10 : objValue};
		_this._props.h[key] = _range;
		if(_this._isFrom) {
			_this.updateProperties(0);
		}
		...
		_this._transform.rotate.degree = 10;
		if(x != null) {
		...
		var _trans = _this._target.getAttribute(_this.TRANSFORM);
		if(_trans.indexOf(_this.ROTATE) == -1) {
			_this._target.setAttribute(_this.TRANSFORM,"" + _trans + " rotate(0)");
		}
		var _this1 = _this;
		...
		var _go = new GoSVG(rocket,4);
		_go._isFrom = false;
		var _this = _go;
		var objValue = 0.0;
		if(_this._target.hasAttribute("transform-y")) {
			objValue = parseFloat(_this._target.getAttribute("transform-y"));
		}
		var _range = { key : "transform-y", from : _this._isFrom ? -100 : objValue, to : !_this._isFrom ? -100 : objValue};
		_this._props.h["transform-y"] = _range;
		if(_this._isFrom) {
			_this.updateProperties(0);
		}
		if(_this._transform.translate == null) {
		...
		_this._transform.translate.y = -100;
		var _this1 = _this;
		_this1._isYoyo = true;
		var _this = _this1;
		_this._easing = cc_lets_easing_Quad.get_easeInOut();
		var _this1 = _this;
		_this1._options.onComplete = function() {
			$global.console.log(rocket);
		};
		_this1._options.onCompleteParams = null;
	}
	,__class__: MainGoSVG
